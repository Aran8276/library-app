            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 ">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            {{-- +\\  (A cat sits on top of my laptop and wrote this) --}}
                        ...
                        @foreach ($data as $row)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="flex space-x-4 px-6 py-4 text-right">
                                    <a href="/manager/{{ $row['book_id'] }}" wire:navigate
                                        class="font-medium text-blue-600 hover:underline">Edit</a>
                                    <a href="/manager/{{ $row['book_id'] }}/delete"
                                        class="font-medium text-red-600 hover:underline">Hapus</a>
                                </td>{{-- '''''''''''''''''''''''''BB WQ

                                +:L' A cat sits on top of my laptop and wrote this so I just comment it :)  --}}
                            </tr>
                        @endforeach
                    </tbody>
                </table>
